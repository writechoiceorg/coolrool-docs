...
## 1. Create an Event
Before you can manage carpools, you need an create event using the `POST /event` endpoint. An event may include details such as date, time, location, and the club ID it’s associated with. You can also decide if you want to send out immediate notifications to potential attendees.
...
- **GET /event/driver-list**: List all drivers for an event, their seats, and assigned passengers.  
- **GET /event/statistic**: Summaries like how many seats are open or how many participants have confirmed.  
- **PATCH /event/driver-position**: Update a driver’s real-time location (optional advanced feature).  

These calls enable continuous monitoring of rides until everyone arrives safely.

## Error Handling & Edge Cases

- **400 Bad Request**: Missing or invalid data (e.g., booking seats for a user not in the event).  
- **409 Conflict**: Trying to book seats when the car is already full.  
- **401 Unauthorized**: Only authenticated users with valid tokens can create or join events.  
- **404 Not Found**: The event or driver car ID doesn’t exist.  
- **Waitlist**: If all seats are taken, participants can either remain unassigned or go into a queue.
