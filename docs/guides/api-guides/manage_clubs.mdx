...
```
1. Create a New Club  ->  2. List or Search Clubs  ->  3. Import Members  ->  4. Manage Membership  ->  5. Suspend or Reactivate a Club
```

### 1. Create a New Club
Before you can add members or manage rosters, you need a club record in COOLROOL using the [`POST /clubs`](/reference/endpoints/clubs/create-club) endpoint. Once created, each club can be enriched with details like logo, social media links, and contact information.
...
- **GET /clubs/memberships**: Retrieve membership info.
- **DELETE /clubs/memberships**: Remove membership from a club (requires specifying relevant IDs).
