...
- `GET /sponsor/club/{clubId}` to fetch all sponsors for a particular club.  
- `GET /sponsor/recommendation`, `GET /sponsor/global`, or `GET /sponsor/local` for more targeted sponsor groupings.
...
- `PATCH /sponsor/{sponsorId}` – Update sponsor details (e.g., name, contact info, or add/remove clubs).  
- `DELETE /sponsor/{sponsorId}` – Remove the sponsor from COOLROOL entirely.
