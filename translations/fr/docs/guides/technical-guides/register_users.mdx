...
Une fois le numéro de téléphone vérifié, vous pouvez vous connecter en utilisant le point de terminaison [`POST /auth/login`](/reference/endpoints/auth/login). L'exemple suivant montre comment se connecter à un utilisateur:
